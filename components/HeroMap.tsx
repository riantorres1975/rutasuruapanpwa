const SUGGESTION = {
  ...
  title: "Camión + Teleférico",
  meta: "Costo estimado por abordaje: $11.00 MXN"
} as const;
...
export default function HeroMap() {
  return (
    <div className="hero-map-frame aspect-[4/5] w-full max-w-md lg:aspect-[3/4]" aria-label="Vista previa de rutas de transporte en Uruapan">
      ...
      <svg className="absolute inset-0 h-full w-full" viewBox="0 0 430 520" fill="none" aria-hidden="true">
        <defs>
      ...
      <div className="absolute bottom-4 left-4 right-4 z-10 rounded-2xl border border-cream-100/15 bg-ink-900/85 p-4 backdrop-blur-xl">
        ...
        <div className="mt-3 flex items-center gap-1.5" aria-hidden="true">
          <span className="h-1 w-6 rounded-full bg-terracota-400" />
          <span className="h-1 w-1 rounded-full bg-cream-100/20" />
          <span className="h-1 w-1 rounded-full bg-cream-100/20" />
        </div>
      </div>
    </div>
